    ...
    CALL PSTRANSP(iday,ihour,RDFIPT,TUIPT,TDIPT,RNET,WIND,PAR,TAIR,TMOVE,CA,RH,VPD,VMFD,PRESS,JMAX25,&
        ...
        SMD1,SMD2,WC1,WC2,SOILDATA,SWPEXP,FSOIL,G0,D0L,GAMMA,VPDMIN,G1,GK,WLEAF,NSIDES,   &
        VPARA,VPARB,VPARC,VFUN,SF,PSIV,ITERMAX,GSC,ALEAF,IJV,RD,ET,FHEAT,TLEAF,GBH,PLANTK,TOTSOILRES,MINLEAFWP,   &
        WEIGHTEDSWP,KTOT,HMSHAPE,PSILIN,PSIL,ETEST,CI,ISMAESPA,CDJ,CDV)
...
100 CONTINUE  ! Return point for iterations
    ...
    CALL PHOTOSYN(PAR,TLEAF,TMOVE,CS,RHLEAF,DLEAF,VMLEAF,JMAX25,IECO,EAVJ,EDVJ,DELSJ,VCMAX25,&
        ...
        VPARA,VPARB,VPARC,VFUN,SF,PSIV,HMSHAPE,PSILIN,PSIL,CI,ISMAESPA,CDJ,CDV)
...
200 FHEAT = RNET - LHV*ET
    ...
    JMAXD  = JMAX25  
     ...
     ! Effect of VPD on Vcmax and Jmax. JY MAY/2019
    VCMAXD = VCMAXD * (1.0 - CDV * VPD / 1000.0)
    JMAXD  = JMAXD  * (1.0 - CDJ * VPD / 1000.0)
    
    IF (VCMAXD.LE.10.0) THEN
        VCMAXD = 10.0
        RETURN
    END IF
    
    IF (JMAXD.LE.17.0) THEN
        JMAXD = 17.0
        RETURN
    END IF
    ...
    JMAX = JMAXTFN(JMAXD,TLEAF,EAVJ,EDVJ,DELSJ,TVJUP,TVJDN)      ! Po
    VCMAX = VCMAXTFN(VCMAXD,TLEAF,EAVC,EDVC,DELSC,TVJUP,TVJDN)   ! Ma
    RD = RESP(RD0,RD0ACC,TLEAF,TMOVE,Q10F,K10F,RTEMP,DAYRESP,TBELOW)

    !! Effect of soil water stress on Vcmax and Jmax.  Not needed for now JY MAY/2019
    !IF(ISMAESPA)THEN
    ...
    IF(ISMAESPA)THEN
        IF(WSOILMETHOD.EQ.1.AND.MODELGS.NE.6)THEN
            ...
            IF(ETEST > EMAXLEAF)THEN
                ...
                ALEAF = AMIN1(AC,AJ)  - RD     ! Emax model solution. !Rd subtracted, BM 02/19
                IF (AC < AJ) THEN
                    IJV = 1
                END IF
